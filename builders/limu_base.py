...
mount --bind /dev/pts /mnt3/dev/pts
mount --bind /proc /mnt3/proc
mount --bind /tmp /mnt3/tmp
mount --bind /sys /mnt3/sys

chroot /mnt3 /usr/bin/setuidgid %(uid)s:%(gid)s /usr/bin/env -i RESOURCES=/ HOME=/scratch TERM="$TERM" PS1='\u:\w$ ' PATH=/bin:/usr/bin:/sbin:/usr/sbin%(path)s /user.sh || (echo 'build failed, starting debugging shell'; sh)
...
def Build():
    ...
    d['uid'] = Option('uid', 1000)
    d['gid'] = Option('gid', 1000)
